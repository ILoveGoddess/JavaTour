				...
				<td>
					<table width="100%" border="0" align="center" cellpadding="0"
						...
						<tr>
							...
							<td width="98%">
							<span class="sys_list_yh">网点&gt;&gt;修改设备报修</span>
							</td>
						</tr>
					 </table>
					 <br />
					
				       <form action="../wd/wdupdate.do" method="post" onsubmit="return ckick();" >
				          <input type="hidden"  name="po.repairid"  value="${rvm.repairId }" >
					      <input type="hidden"  name="pageinfo.curpage"  value="${curpage}" >
					         ...
					         <%--<s:select id="sbzl" name="po.equipmenttype.equipmentId"   list="#request.sbzls"  listKey="equipmentId"  listValue="equipmentName" theme="simple" onchange="checksbmx();" ></s:select>
					          ...
					          <select id="sbzl" name="po.equipmenttype.equipmentId" onchange="checksbmx();">
					          	<c:forEach items="${equTypes}" var = "equType">
					          		<option value="${equType.equipmentId }" <c:if test="${equType.equipmentId==rvm.equipmentId }">selected</c:if>> ${equType.equipmentName }</option>
					          	</c:forEach>
					          </select>
					          <%--<input id="wdxx" type="hidden" name="ss" value="${wdid }" >
					       --%></td>
					     </tr>
					     <tr>
					       <td>设备流水号</td>
					       <td>
					         <select id="sbmx" name="po.bankEquipment.equipmenteachId" >
					          <c:forEach items="${eachIds}" var="id">
					           <option   value="${id }"   <c:if test="${id==rvm.equipmentEachId}">selected="selected"</c:if>  >${id }</option>  
					         </c:forEach>
					        </select>
					       </td>
					     </tr>
